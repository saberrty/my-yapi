...
class InterfaceEditForm extends Component {
    ...
    try {
      this.props.form.validateFields((err, values) => {
        ...
        if (!err) {
          ...
          values.resp_demo = this.state.resp_demo;
          values.head_demo = this.state.head_demo;
          values.body_demo = this.state.body_demo;
          values.query_demo = this.state.query_demo;
  ...
  renderJsonEditor(type) {
    console.info(type);
    const key = `${type}_demo`;
    const data = (this.state[key] && JSON.parse(this.state[key])) || null;
    return (
      <AceEditor
        ...
        onChange={e => {
          ...
          const stateValue = objType === "string" ? obj : JSON.stringify(obj);
          switch (type) {
            case "resp":
              this.setState({
                resp_demo: stateValue,
              });
              break;
            case "body":
              this.setState({
                body_demo: stateValue,
              });
              break;
            case "head":
              this.setState({
                head_demo: stateValue,
              });
              break;
            case "query":
              this.setState({
                query_demo: stateValue,
              });
              break;
            default:
              break;
